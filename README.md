...
This will guide you to the prerequisites, steps to run the application , faqs related to the appliation. before continuing I will state my assumptions.
...
There are cretain prerequisites for the project to be able to run on the system.
 ...
 1. The softwares we need to make sure the application run on your machine correctly are as follows : 
      * python3.6.8 32 bit version
      * Pywin32 Package 
      * qt4 package 
      * OPC DA COM component is opcdaauto.dll x86 version.(I am refering the folders in the package  here )
...
1. if you get all the required softwarers successfully lets install them one by one with a procedure so that yoou may not fail at any step 
   ...
   1. register the dll with the below steps
     ...
     1.open an elevated command window (i.e right click and run as administartor) and run the below one 
        ```cmd
